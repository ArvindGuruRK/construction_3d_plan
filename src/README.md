...
graph TD
    ...
    D -- Total Area, Room Counts --> E[Config Panel State];
    E --> F[User Clicks "Generate Plans"];

    subgraph Frontend Logic
        F --> G[onSubmit Function in page.tsx];
        G --> H{Set Loading State to `true`};
        H --> I[Capture Form Data];
        I --> J{Pass Form Data to ViewerGrid};
    end

    subgraph 3D Generation & Rendering
        J --> K[ViewerGrid Renders ModelCard(s)];
        K -- planConfig prop --> L[ModelCard Component];
        L -- planConfig prop --> M[InteractiveViewer Component];
        M --> N[FloorPlan Component Receives Config];
        N --> O{Procedural Layout Algorithm Runs};
        O -- Generates Room Positions & Dimensions --> P[3D Geometry is Constructed];
        P -- Walls, Floor, Labels --> Q[React Three Fiber Canvas Renders Scene];
    end

    Q --> R{User Interacts with 3D Model};
    R -- Orbit, Pan, Zoom --> Q;
    R -- Clicks "Regenerate" --> S[Regeneration Key is Updated];
    S --> N;
    R -- Clicks "Download" --> T[GLTFExporter Captures Scene];
    T --> U[User Downloads .gltf File];
...
2.  **Server Actions (Currently Mocked)**: The file `src/app/actions.ts` is structured as a Next.js Server Action. In a server-connected version of this app, this is where the client would securely call backend logic (like a Genkit AI flow). In the current implementation, it serves as a placeholder that simulates a network request, demonstrating a modern Next.js pattern for client-server communication.

3.  **Modular Component Structure**: The application is broken down into highly specific, reusable components. UI components are separated from 3D components, and application-specific components (`/arch-ai`) are distinct from generic ones (`/ui`), making the codebase easier to maintain.
...
- **`onSubmit` Logic**: This function orchestrates the generation process. It sets the loading state, captures the form data, and (after a simulated delay) populates the `results`, triggering a re-render to display the `ViewerGrid`.

### `components/arch-ai/config-panel.tsx`
This component renders the form controls in the sidebar.
- It receives the `form` instance from `page.tsx` as a prop.
- It uses ShadCN's `<Form />` and `<FormField />` components to connect inputs to the `react-hook-form` state.
- **Controls**:
    - `Slider`: For selecting the total area.
    - `CounterInput`: For selecting the number of each room type.
...
- **Scene Setup**:
    ...
    - **Helpers**: A `gridHelper` is included to provide a sense of scale and orientation.
- **`FloorPlan` Component**: This is where the procedural logic resides.
    - **`generateLayout` Function**: This is the core algorithm. It takes the user's `planConfig` and performs a series of calculations to decide where each room should go and how large it should be. It uses a simple "greedy packing" algorithm: it places rooms one by one in a grid-like fashion until the total area is filled.
    - **Geometry Construction**: It iterates over the generated layout data and constructs the 3D geometry:
        - **Floor**: A simple `<planeGeometry>`.
        - **Walls**: Uses an `<extrudeGeometry>` with a custom `Shape` that includes holes for doors and windows. This is more complex but far more realistic than simple boxes.
        - **Labels**: Uses the `<Text>` component from `@react-three/drei` to place the name of each room within its 3D space.

### `components/arch-ai/model-card.tsx`
This component acts as a container for each generated 3D model variation.
- **State**: It manages a `regenerationKey`. When the "Regenerate" button is clicked, this key is updated. Passing this key to the `InteractiveViewer` forces React to re-mount the component, which in turn re-runs the procedural generation algorithm to create a new, unique layout.
- **Scene Handling**:
    - It maintains a `sceneRef` to get a direct handle on the underlying Three.js `Scene` object.
    - **Download Logic**: The `handleDownload` function uses this `sceneRef` to pass the scene to a `GLTFExporter`. This exporter traverses the scene graph and converts it into the `.gltf` format, which is then triggered as a file download for the user.
